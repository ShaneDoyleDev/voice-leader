...
- **Checking draws the voices.** Hitting Check strokes in a faint line from each voice of every chord to the same voice in the next, chord by chord across the progression — four lines per move, so you see the motion rather than inferring it. Crossed voices make crossed lines. Any pair the checklist catches moving in parallel fifths or octaves draws in red at full strength instead, so the offending two lines are visible among the rest. The lines belong to the report, so editing any note takes them away with it.
