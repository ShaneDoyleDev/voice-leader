...
- **Four voices to a chord.** Each chord takes at most four notes, SATB. Once it has them the ghost preview stops offering a fifth and the column says “4 voices” above it, and a note dragged in from elsewhere is turned away. Clicking that label turns it into a cross; clicking the cross empties that chord, so a voicing can be restarted without clearing the whole staff.
