...
- **The checklist.** Once every chord has at least three notes, a **Check voice leading** button appears. A chord written in three parts is allowed — it simply says so, since spacing, crossing and the parallel checks all want a fourth voice. Voices come from the opening chord as described above — the report marks what to fix in red, what to watch in amber, and what is merely worth noting in green:
  - **Completeness** — the root and the third must be present somewhere in the four voices; missing either is an error. The fifth may be left out: with the root tripled that is noted as the usual way to do it, and without the tripling it is a caution rather than an error. A diminished triad is the exception — vii°, and ii° in minor, must keep the fifth, since the diminished fifth is the interval the chord is named for.
